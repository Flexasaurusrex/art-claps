...
export async function GET(request: NextRequest) {
  try {
    ...
    let pointsColumn: keyof LeaderboardUser = 'totalPoints';
    ...
    // Add rankings to users with proper typing
    const rankedUsers = (users as LeaderboardUser[]).map((user, index) => ({
      ...user,
      rank: offset + index + 1,
    ...
    const totalPointsAwarded = stats?.reduce((sum, user) => {
      const points = user[pointsColumn] as number;
      return sum + (points || 0);
    }, 0) || 0;
